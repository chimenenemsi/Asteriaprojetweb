...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        @media (max-width:900px){.monta-hero,.monta-grid,.grid{grid-template-columns:1fr}.monta-navbar,.monta-topbar-inner,.monta-footer-inner{flex-direction:column;align-items:flex-start}.monta-topbar-left,.monta-topbar-right,.monta-nav{flex-wrap:wrap;justify-content:flex-start}.col-6,.col-4,.col-3{grid-column:span 12}}
    </style>
</head>
<body>
    <?php
    ...
    $switchRoute = in_array($section, ['goals', 'records'], true) ? 'backoffice/' . $section : 'backoffice/dashboard';
    ?>
        ...
        <div class="monta-shell monta-navbar">
            ...
            <nav class="monta-nav">
                <?php foreach (frontoffice_navigation() as $slug => $label): ?>
                    ...
                    <?php $isActive = $section === $slug || ($slug === 'home' && $currentRoute === 'frontoffice/home'); ?>
    ...
    <style>
        .progress-ai-survey-float{position:fixed!important;right:24px!important;bottom:24px!important;z-index:2147483000!important;display:inline-flex!important;align-items:center!important;gap:10px!important;text-decoration:none!important;border-radius:999px!important;background:linear-gradient(135deg,#6ca138,#2d4a1e)!important;color:#fff!important;font-weight:900!important;padding:16px 22px!important;box-shadow:0 18px 42px rgba(45,74,30,.32)!important;border:2px solid rgba(255,255,255,.55)!important}
        .progress-ai-survey-float:hover{transform:translateY(-1px)!important;color:#fff!important}
        .progress-ai-survey-float::before{content:'AI';display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:999px;background:rgba(255,255,255,.18);font-size:12px}
    </style>
    <a class="progress-ai-survey-float" href="<?= htmlspecialchars(route_url('frontoffice/ai-survey'), ENT_QUOTES, 'UTF-8') ?>" aria-label="Open AI Goal Survey">AI Goal Survey</a>
