...
.progress-hero{padding:28px;border-radius:28px;background:radial-gradient(circle at top right,#dcfce7 0%,#e8f5e0 35%,#ffffff 75%);border:1px solid #c8e0be;box-shadow:0 22px 60px rgba(15,23,42,.08)}
.progress-hero h1{margin:0;color:#153122}
.progress-hero-copy{max-width:760px;color:#60706a;line-height:1.7;margin-top:10px}
.progress-stat-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
.progress-stat-card{padding:20px;border-radius:22px;background:linear-gradient(180deg,#ffffff 0%,#f7fbf4 100%);border:1px solid #d4e8cc;box-shadow:0 16px 40px rgba(15,23,42,.06)}
.progress-stat-card strong{display:block;font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#60706a}
.progress-stat-card span{display:block;font-size:32px;font-weight:800;color:#153122;margin-top:8px}
.progress-stat-card small{display:block;margin-top:8px;color:#60706a;line-height:1.5}
...
.goal-card h3{margin:0;color:#153122;font-size:20px;line-height:1.3}
.goal-card p{margin:0;color:#60706a;line-height:1.6}
.goal-chip{display:inline-flex;align-items:center;padding:7px 11px;border-radius:999px;background:rgba(108,161,56,.12);color:#3a6b2a;font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}
...
.goal-meta-item strong{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#60706a}
.goal-meta-item span{display:block;margin-top:6px;font-weight:700;color:#153122}
.goal-card .actions{display:flex;gap:10px;flex-wrap:wrap}
.goal-empty{padding:28px;color:#60706a}
...
.progress-ai-inline-card{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:20px 22px;border-radius:24px;background:linear-gradient(135deg,#1b2115,#2d4a1e);color:#fff;box-shadow:0 22px 60px rgba(15,23,42,.12)}.progress-ai-inline-card strong{display:block;font-size:20px;margin-bottom:6px}.progress-ai-inline-card span{display:block;color:rgba(255,255,255,.75);line-height:1.6}.progress-ai-inline-card .btn{background:#6ca138;color:#fff;white-space:nowrap}@media(max-width:800px){.progress-ai-inline-card{flex-direction:column;align-items:flex-start}}
</style>
...
<div id="goals-live-panel" class="progress-dashboard-shell">
    <div class="progress-hero">
        <div class="header-line">
            ...
            <div class="actions">
                <?php if (($area ?? 'frontoffice') === 'frontoffice'): ?>
                    <a class="btn btn-primary" href="<?= htmlspecialchars(route_url('frontoffice/ai-survey'), ENT_QUOTES, 'UTF-8') ?>">Start AI Survey</a>
                <?php endif; ?>
    ...
    <?php if (($area ?? 'frontoffice') === 'frontoffice'): ?>
        <div class="progress-ai-inline-card">
            <div>
                <strong>Need help choosing the right target?</strong>
                <span>Use Gemini to recommend a goal based on your focus, level, schedule, limitations, and timeframe.</span>
            </div>
            <a class="btn btn-primary" href="<?= htmlspecialchars(route_url('frontoffice/ai-survey'), ENT_QUOTES, 'UTF-8') ?>">Open AI Goal Survey</a>
        </div>
    <?php endif; ?>
    ...
    <div class="progress-filters">
        <form method="get" action="<?= htmlspecialchars(base_url() . '/index.php', ENT_QUOTES, 'UTF-8') ?>" novalidate class="form-shell" data-live-search="true">
            ...
            <div class="actions">
                ...
                <button class="btn btn-primary" type="submit">Search goals</button>
