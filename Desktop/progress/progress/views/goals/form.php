<?php
$aiPrefill = [
    'title' => trim((string) ($_GET['prefill_title'] ?? '')),
    'metric' => trim((string) ($_GET['prefill_metric'] ?? '')),
    'unit' => trim((string) ($_GET['prefill_unit'] ?? '')),
    'goal_type' => trim((string) ($_GET['prefill_goal_type'] ?? '')),
    'start_value' => trim((string) ($_GET['prefill_start_value'] ?? '')),
    'target_value' => trim((string) ($_GET['prefill_target_value'] ?? '')),
    'target_date' => trim((string) ($_GET['prefill_target_date'] ?? '')),
    'description' => trim((string) ($_GET['prefill_description'] ?? '')),
];
?>
<?php
$values = $values ?? [];
// Apply AI survey prefill values only when the form is opening empty/new.
foreach (($aiPrefill ?? []) as $prefillKey => $prefillValue) {
    if ($prefillValue !== '' && (($values[$prefillKey] ?? '') === '' || in_array($prefillKey, ['target_date', 'description'], true))) {
        $values[$prefillKey] = $prefillValue;
    }
}
$errors = $errors ?? [];
$mode = $mode ?? 'create';
    ...
    <form method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>" novalidate class="form-shell">
            ...
            <div class="row">
                <div class="col-6">
                    <div class="field">
                        ...
                        <input id="goal-title" type="text" name="title" placeholder="Goal title" value="<?= htmlspecialchars((string) ($values['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-6">
                    <div class="field">
                        ...
                        <input id="goal-metric" type="text" name="metric" placeholder="Weight, sales, steps, water intake..." value="<?= htmlspecialchars((string) ($values['metric'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-4">
                    <div class="field">
                        ...
                        <input id="goal-unit" type="text" name="unit" placeholder="kg, %, liters..." value="<?= htmlspecialchars((string) ($values['unit'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-4">
                    <div class="field">
                        ...
                        <select id="goal-status" name="status">
                            <?php foreach (['ACTIVE' => 'Active', 'COMPLETED' => 'Completed', 'ON_HOLD' => 'On Hold'] as $status => $statusLabel): ?>
                                <option value="<?= htmlspecialchars($status, ENT_QUOTES, 'UTF-8') ?>" <?= ($values['status'] ?? 'ACTIVE') === $status ? 'selected' : '' ?>><?= htmlspecialchars($statusLabel, ENT_QUOTES, 'UTF-8') ?></option>
                            <?php endforeach; ?>
                        </select>
            ...
            <div class="row">
                <div class="col-3">
                    <div class="field">
                        ...
                        <input id="goal-start-value" type="text" inputmode="decimal" name="start_value" placeholder="Example: 90.5" value="<?= htmlspecialchars((string) ($values['start_value'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-3">
                    <div class="field">
                        ...
                        <input id="goal-target-value" type="text" inputmode="decimal" name="target_value" placeholder="Example: 75.0" value="<?= htmlspecialchars((string) ($values['target_value'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-3">
                    <div class="field">
                        ...
                        <input id="goal-start-date" type="text" name="start_date" placeholder="YYYY-MM-DD" value="<?= htmlspecialchars((string) ($values['start_date'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                ...
                <div class="col-3">
                    <div class="field">
                        ...
                        <input id="goal-target-date" type="text" name="target_date" placeholder="YYYY-MM-DD" value="<?= htmlspecialchars((string) ($values['target_date'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                        <div class="field-help">Use the `YYYY-MM-DD` format. Validation is checked in PHP.</div>
        ...
        <div class="form-section">
            ...
            <div class="field">
                ...
                <textarea id="goal-description" name="description" rows="5" placeholder="Add context, milestones, or any useful notes for this goal."><?= htmlspecialchars((string) ($values['description'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
