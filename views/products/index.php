            ...
            <div class="col-2">
                <div class="field">
                    ...
                    <select id="product-sort" name="sort">
                        ...
                        <option value="price_asc" <?= $productFilters['sort'] === 'price_asc' ? 'selected' : '' ?>>DT+</option>
                        <option value="price_desc" <?= $productFilters['sort'] === 'price_desc' ? 'selected' : '' ?>>DT-</option>
    ...
    <div class="product-dashboard-grid">
        <div class="sales-hero">
            ...
            <div class="summary">
                <div class="summary-item">
                    ...
                    <span>DT <?= htmlspecialchars(number_format((float) ($productStats['total_revenue'] ?? 0), 2), ENT_QUOTES, 'UTF-8') ?></span>
                </div>
                <div class="summary-item">
        ...
        <div class="health-list">
            <div class="health-item">
                ...
                <span>DT <?= htmlspecialchars(number_format((float) ($productStats['inventory_value'] ?? 0), 2), ENT_QUOTES, 'UTF-8') ?> currently sitting in stock.</span>
            </div>
            <div class="health-item">
                <strong>Average Order Value</strong>
                <span>DT <?= htmlspecialchars(number_format((float) ($productStats['average_order_value'] ?? 0), 2), ENT_QUOTES, 'UTF-8') ?> per completed non-cancelled order line.</span>
            </div>
            <div class="health-item">
                ...
                <tbody>
                    <?php foreach ($topProducts as $topProduct): ?>
                        <tr>
                            ...
                            <td>DT <?= htmlspecialchars(number_format((float) $topProduct['total_revenue'], 2), ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= htmlspecialchars((string) $topProduct['stock_quantity'], ENT_QUOTES, 'UTF-8') ?></td>
                            <td><?= product_status_badge((string) $topProduct['status']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
            ...
            <tbody>
                <?php foreach ($products as $product): ?>
                    <tr>
                        ...
                        <td><?= product_status_badge((string) $product['status']) ?></td>
                        <td>
                            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                                <a class="btn" style="padding:7px 14px;font-size:13px" href="<?= htmlspecialchars(route_url($area . '/products/edit', ['id' => (int) $product['id']]), ENT_QUOTES, 'UTF-8') ?>">Edit</a>
                                <form class="inline" method="post" action="<?= htmlspecialchars(route_url($area . '/products/delete', ['id' => (int) $product['id']]), ENT_QUOTES, 'UTF-8') ?>" novalidate>
                                    <button class="btn btn-danger" style="padding:7px 14px;font-size:13px" type="submit">Delete</button>
                                </form>
                                <a class="btn" style="padding:7px 14px;font-size:13px;background:#f0f2ed;color:#1b2115" href="<?= htmlspecialchars(route_url($area . '/orders/new', ['product_id' => (int) $product['id']]), ENT_QUOTES, 'UTF-8') ?>">+ Order</a>
                            </div>
                        </td>
                    </tr>
                <?php endforeach; ?>
        ...
        <?php foreach ($products as $product): ?>
            ...
            <div class="catalog-card">
                <div class="catalog-top">
                    ...
                    <div class="catalog-price">DT <?= htmlspecialchars(number_format((float) $product['price'], 2), ENT_QUOTES, 'UTF-8') ?></div>
                </div>
                ...
                <div class="catalog-meta">
                    ...
                    <div class="catalog-meta-item">
                        ...
                        <span><?= product_status_badge((string) $product['status']) ?></span>
                    </div>
                    <div class="catalog-meta-item">
                        <strong>Stock</strong>
                        <?php
                        $stockQty = (int) $product['stock_quantity'];
                        $stockColor = $stockQty < 1 ? '#991b1b' : ($stockQty <= 5 ? '#92400e' : '#166534');
                        $stockBg    = $stockQty < 1 ? '#fee2e2' : ($stockQty <= 5 ? '#fef9c3' : '#dcfce7');
                        $stockLabel = $stockQty < 1 ? 'Out of Stock' : ($stockQty <= 5 ? 'Low ('.$stockQty.')' : 'In Stock ('.$stockQty.')');
                        ?>
                        <span style="font-size:13px;font-weight:700;padding:4px 8px;border-radius:8px;background:<?= $stockBg ?>;color:<?= $stockColor ?>"><?= htmlspecialchars($stockLabel, ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                    <div class="catalog-meta-item">
