...
class ProgressGoalController extends BaseController
{
    ...
    {
        $tags = match ($type) { 'discipline' => ['success|inspirational', 'motivational', 'wisdom'], 'confidence' => ['motivational', 'success|inspirational', 'future'], 'resilience' => ['wisdom', 'motivational', 'success|inspirational'], default => $this->goalTags($goal) };
        foreach ($tags as $tag) {
            try {
                $payload = $this->requestJson(self::QUOTE_URL . '?tags=' . rawurlencode($tag));
                if (is_array($payload) && isset($payload['content'], $payload['author'])) { return ['content' => (string) $payload['content'], 'author' => (string) $payload['author'], 'tag' => $type]; }
            } catch (Throwable) {
                continue;
            }
        }
        return ['content' => 'The next useful win is usually one honest step beyond your last result.', 'author' => 'Asteria', 'tag' => $type];
    }

    private function goalTags(ProgressGoal $goal): array
    {
        return match ($goal->getGoalType()) { 'WEIGHT_LOSS', 'FITNESS', 'HEALTH' => ['motivational', 'success|inspirational', 'wisdom'], 'CAREER', 'PRODUCTIVITY', 'FINANCE' => ['success|inspirational', 'future', 'motivational'], 'LEARNING' => ['education', 'wisdom', 'motivational'], default => ['motivational', 'success|inspirational', 'wisdom'] };
    }
