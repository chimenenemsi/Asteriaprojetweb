    ...
    protected function requestJson(string $url): mixed
    {
        if (function_exists('curl_init')) {
            ...
            if ($response === false) {
                ...
                curl_close($handle);
                throw new RuntimeException($message !== '' ? $message : 'Unable to reach the remote service.');
            }
            ...
            if ($response === false) {
                throw new RuntimeException('Unable to reach the remote service.');
            }
        }
    ...
    private function buildPdfPageStreams(string $title, array $metaLines, array $bodyLines): array
    {
        ...
        foreach ($pageStreams as $pageNumber => $pageStream) {
            $pageStreams[$pageNumber] = $pageStream
                ...
                . $this->pdfTextLine($margin, 22.0, 'F1', 9.0, 'Generated by Progress export', [0.48, 0.54, 0.63])
