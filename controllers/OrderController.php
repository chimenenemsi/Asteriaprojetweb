...
class OrderController extends BaseController
{
    ...
    {
        if ($area === 'frontoffice' && !isset($_SESSION['user'])) {
        ...
        $this->render('orders/index', [
            ...
            'orders' => $this->allOrders($filters, $this->currentUserFilter($area)),
            'orderFilters' => $filters,
            'products' => $this->allProducts(),
            'orderStats' => $this->orderStatistics($filters, $this->currentUserFilter($area)),
        ], $area);
    }
    ...
    public function exportPdf(string $area): void
    {
        ...
        foreach ($orders as $order) {
            $lines[] = sprintf(
                'User #%s %s | %s | %s | qty %d | $%0.2f | %s | %s',
                (string) (($order['user_id'] ?? '') ?: 'guest'),
                (string) (($order['user_fullname'] ?? '') ?: ''),
    ...
    public function show(string $area): void
    {
        ...
        if ($order === null || !$this->canAccessOrder($order, $area)) {
            $this->renderNotFound();
            return;
        }
    ...
    public function form(string $area, string $mode): void
    {
        ...
        if ($mode === 'edit' && ($order === null || !$this->canAccessOrder($order, $area))) {
            $this->renderNotFound();
            return;
        }
        ...
        $this->render('orders/form', [
            ...
            'values' => $order ?? [
                ...
                'customer_name' => (string) ($_SESSION['user']['fullname'] ?? ''),
                'customer_email' => (string) ($_SESSION['user']['email'] ?? ''),
                'quantity' => 1,
                'status' => 'PENDING',
    ...
    private function prepare(array $source, string $area, string $mode, ?array $existingOrder = null): array
    {
        ...
        if ($mode === 'create') {
            $data['user_id'] = (int) ($_SESSION['user']['id'] ?? 0) ?: null;
            $data['order_date'] = date('Y-m-d');
            $data['status'] = $area === 'frontoffice'
                ? 'PENDING'
                : ((string) ($data['status'] ?? '') !== '' ? (string) $data['status'] : 'PENDING');
        } else {
            $data['user_id'] = isset($existingOrder['user_id']) && $existingOrder['user_id'] !== null
                ? (int) $existingOrder['user_id']
                : null;
    ...
    private function allOrders(array $filters = [], ?int $userId = null): array
    {
        $query = 'SELECT o.*, p.name AS current_product_name, p.sku AS current_product_sku, '
            ...
            . 'c.name AS current_category_name, u.fullname AS user_fullname, u.email AS user_email FROM orders o '
        ...
        if ($userId !== null) {
            $query .= ' AND o.user_id = :current_user_id';
            $params['current_user_id'] = $userId;
        }

        if (($filters['search'] ?? '') !== '') {
            $searchTerm = '%' . $filters['search'] . '%';
            $query .= ' AND (o.customer_name LIKE :search_customer OR o.customer_email LIKE :search_email OR o.product_name LIKE :search_product OR COALESCE(o.delivery_location_name, "") LIKE :search_location OR COALESCE(u.fullname, "") LIKE :search_user OR COALESCE(u.email, "") LIKE :search_user_email)';
            ...
            $params['search_user_email'] = $searchTerm;
        }
    ...
    {
        $stmt = $this->connection()->prepare(
            'INSERT INTO orders (user_id, product_id, product_name, customer_name, customer_email, quantity, unit_price, total_amount, status, order_date, shipping_address, delivery_location_name, delivery_latitude, delivery_longitude, notes) '
            . 'VALUES (:user_id, :product_id, :product_name, :customer_name, :customer_email, :quantity, :unit_price, :total_amount, :status, :order_date, :shipping_address, :delivery_location_name, :delivery_latitude, :delivery_longitude, :notes)'
        );
    ...
    {
        $stmt = $this->connection()->prepare(
            'UPDATE orders SET user_id = :user_id, product_id = :product_id, product_name = :product_name, customer_name = :customer_name, '
    ...
    private function ensureOrderLocationSchema(): void
    {
        ...
        $this->ensureOrderColumn('user_id', 'ALTER TABLE orders ADD COLUMN user_id INT NULL AFTER id');
        $this->ensureOrderForeignKey('fk_orders_user', 'ALTER TABLE orders ADD CONSTRAINT fk_orders_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL');
    ...
    private function ensureOrderForeignKey(string $constraint, string $sql): void
    {
        $statement = $this->connection()->prepare(
    ...
    private function orderStatistics(array $filters = [], ?int $userId = null): array
    {
    ...
    private function currentUserFilter(string $area): ?int
    {
        if ($area !== 'frontoffice') {
            return null;
        }
    ...
    private function canAccessOrder(array $order, string $area): bool
    {
        if ($area !== 'frontoffice') {
            return true;
        }

        $currentUserId = $this->currentUserFilter($area);

        return $currentUserId !== null && (int) ($order['user_id'] ?? 0) === $currentUserId;
    }
}
