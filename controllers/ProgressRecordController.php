...
class ProgressRecordController extends BaseController
{
    ...
    private const QUOTE_URL = 'https://api.quotable.io/random';
    ...
    public function insights(string $area): void
    {
        ...
        try {
            echo json_encode(['quote' => $this->recordQuote($goal), 'baseline' => $this->weeklyBaseline($goalId, $recordDate)], JSON_THROW_ON_ERROR);
        } catch (Throwable $exception) {
            http_response_code(503);
            echo json_encode(['quote' => ['content' => 'Progress grows when you keep showing up for your next rep.', 'author' => 'Asteria', 'tag' => 'fallback'], 'baseline' => $this->weeklyBaseline($goalId, $recordDate), 'error' => $exception->getMessage()]);
        }
    }
    ...
    private function recordQuote(ProgressGoal $goal): array
    {
        $tags = in_array(strtoupper($goal->getGoalType()), ['WEIGHT_LOSS', 'FITNESS', 'HEALTH'], true) || str_contains(strtolower($goal->getMetric()), 'weight')
            ? ['motivational', 'success|inspirational', 'wisdom']
            : (in_array(strtoupper($goal->getGoalType()), ['CAREER', 'FINANCE', 'PRODUCTIVITY'], true) ? ['success|inspirational', 'future', 'motivational'] : (strtoupper($goal->getGoalType()) === 'LEARNING' ? ['wisdom', 'education', 'motivational'] : ['motivational', 'success|inspirational', 'wisdom']));
        foreach ($tags as $tag) {
            try {
                $payload = $this->requestJson(self::QUOTE_URL . '?tags=' . rawurlencode($tag));
                if (is_array($payload) && isset($payload['content'], $payload['author'])) { return ['content' => (string) $payload['content'], 'author' => (string) $payload['author'], 'tag' => $tag]; }
            } catch (Throwable) {
                continue;
            }
        }
        return ['content' => 'Visualizing your progress is how today starts beating last week.', 'author' => 'Asteria', 'tag' => 'fallback'];
    }
