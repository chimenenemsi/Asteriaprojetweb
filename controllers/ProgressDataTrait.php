...
trait ProgressDataTrait
{
     ...
     * @return ProgressGoal[]
     */
    protected function fetchAllGoals(?int $userId = null): array
    {
        $sql = 'SELECT * FROM progress_goals';
        $params = [];
        ...
        $sql .= ' ORDER BY target_date ASC, id DESC';
    ...
    protected function findGoalById(int $id, ?int $userId = null): ?ProgressGoal
    {
        $sql = 'SELECT * FROM progress_goals WHERE id = :id';
        $params = ['id' => $id];
    ...
    protected function validateGoal(array $input, ?int $userId = null): array
    {
    ...
    protected function goalFromValues(array $values, ?int $id = null, ?int $userId = null): ProgressGoal
    {
    ...
    protected function createGoal(ProgressGoal $goal): int
    {
        $statement = $this->connection()->prepare(
            'INSERT INTO progress_goals (user_id, title, metric, start_value, target_value, unit, start_date, target_date, goal_type, status, description)
            VALUES (:user_id, :title, :metric, :start_value, :target_value, :unit, :start_date, :target_date, :goal_type, :status, :description)'
        );
    ...
    protected function deleteGoal(int $id, ?int $userId = null): void
    {
        $sql = 'DELETE FROM progress_goals WHERE id = :id';
        $params = ['id' => $id];
     ...
     */
    protected function fetchAllRecords(?int $userId = null): array
    {
        $sql = 'SELECT pr.id, pr.progress_goal_id, pr.record_date, pr.recorded_value,
     ...
     * @return ProgressRecord[]
     */
    protected function fetchRecordsByGoal(int $goalId, ?int $userId = null): array
    {
        $sql = 'SELECT pr.id, pr.progress_goal_id, pr.record_date, pr.recorded_value,
            pr.adherence_score, pr.mood, pr.record_type, pr.notes, pr.created_at, pr.updated_at,
            pg.title AS goal_title, pg.unit AS goal_unit
        ...
        $params = ['progress_goal_id' => $goalId];
        if ($userId !== null) {
            $sql .= ' AND pg.user_id = :user_id';
            $params['user_id'] = $userId;
        }
    ...
    protected function findRecordById(int $id, ?int $userId = null): ?ProgressRecord
    {
        $sql = 'SELECT pr.id, pr.progress_goal_id, pr.record_date, pr.recorded_value,
            pr.adherence_score, pr.mood, pr.record_type, pr.notes, pr.created_at, pr.updated_at,
            pg.title AS goal_title, pg.unit AS goal_unit
    ...
    protected function validateRecord(array $input, ?int $userId = null): array
    {
        ...
        if (!ctype_digit($goalId) || (int) $goalId <= 0) {
            ...
            $goal = $this->findGoalById((int) $goalId, $userId);
            if ($goal === null) {
